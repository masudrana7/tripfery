...
class Utility {
	...
	public function register_notice() {
		...
		if ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$message = sprintf(
				...
				__( 'Your license key doesn\'t match your current domain. Please activate the license again for your current domain.', 'tripfery' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'tripfery' ),
			);
		} else {
			$message = sprintf(
				'<b>%1$s <a href="%2$s" target="_blank">%3$s</a> %4$s <a href="%5$s">%6$s</a></b>',
				__( 'Please activate your theme using Envato', 'tripfery' ),
				esc_url( 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ),
				__( 'purchase code', 'tripfery' ),
				__( 'to get full theme functionality and customer support.', 'tripfery' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'tripfery' ),
			);
		}
	...
	public function script() {
				...
				if ( purchase_code ) {
					jQuery.ajax({
						...
						success: function( resp ) {   
							if ( resp === 555 ) {
								alert('<?php esc_html_e( 'Purchase code already activated for one domain!!!', 'tripfery' ); ?>'); 
							} else {
								if ( resp ) {
									jQuery('.rtlc-status-btn').html('<?php esc_html_e( 'Activated', 'tripfery' ); ?>');
									jQuery('.rtlc-status-btn').removeClass('rtlc-unverified').addClass('rtlc-verified');
								} else {
									alert('<?php esc_html_e( 'Sorry!!! Purchase code does not match', 'tripfery' ); ?>');
								}
							} 
					...
					alert('<?php esc_html_e( 'Purchase code is required!!!', 'tripfery' ); ?>');
				} 
			});
		</script> 
			<?php
		...
		if ( wp_script_is( 'jquery', 'done' ) && ! rtlc_is_valid()['success'] ) {
			if ( isset( $_GET['page'] ) && 'fw-backups-demo-content' === $_GET['page'] ) {
				...
				<script type="text/javascript"> 
					jQuery("#fw-ext-backups-demo-list .theme-actions a").on("click", function(e) { 
						e.preventDefault();
						alert('<?php esc_html_e( 'Please activate your theme using Envato purchase code, to install demo data.', 'tripfery' ); ?>');
						return false;
					});
				</script> 
				<?php
			...
			if ( isset( $_GET['page'] ) && 'tripfery-install-plugins' === $_GET['page'] ) {
				?>
				<script type="text/javascript">  
					jQuery(".row-actions a").on("click", function(e) {  
						if ( jQuery(this).closest('td').next().has('span').length > 0 ) { //find pre packaged
							e.preventDefault();
							alert('<?php esc_html_e( 'Please activate your theme using Envato purchase code, to use this plugin.', 'tripfery' ); ?>');
							return false;
						} 
					});
...
if ( ! function_exists( 'rtlc_is_valid' ) ) {
	...
	function rtlc_is_valid() {
		...
		$tripfery = $theme_info->get( 'Name' );

		$tripfery = strtolower( trim( preg_replace( '/[^A-Za-z0-9-]+/', '-', $tripfery ) ) );
		$get_option      = get_option( 'rt_licenses' );

		if ( isset( $get_option[ $tripfery . '_license_key' ] ) ) {
			return [
				'success'      => true,
				'domain_match' => true,
			];
		} elseif ( isset( $get_option[ $tripfery . '_license' ] ) && isset( $get_option[ $tripfery . '_license' ]['key'] ) ) {
			$domain_name = rtlc_get_domain_name();
			$domain      = $get_option[ $tripfery . '_license' ]['domain'];
