...
class Helper {
	...
	public function theme_menu() {
		add_theme_page( esc_html__( 'Theme License', 'tripfery' ), esc_html__( 'Theme License', 'tripfery' ), 'manage_options', 'rtlc', [ $this, 'create_admin_page' ], null, 99 );
	}
	...
	public function create_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Theme License', 'tripfery' ); ?></h1>
			<form method="post" action="options.php">
			<?php
				// This prints out all hidden setting fields.
				settings_fields( 'rt_option_group' );
				do_settings_sections( 'tripfery-setting' );
			?>
			</form>
		</div>
		<?php
	...
	public function theme_option() {
		...
		add_settings_field(
			'rt_purchase_code',
			esc_html__( 'Purchase Code', 'tripfery' ),
		...
		add_settings_field(
			'rt_license_status',
			esc_html__( 'License Status', 'tripfery' ),
	...
	public function license_status_callback() {
		...
		$status_text = esc_html__( 'Not Activated', 'tripfery' );

		if ( rtlc_is_valid()['success'] ) {
			$verify      = true;
			$status_text = __( 'Activated', 'tripfery' );
		} elseif ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$status_text = __( 'Domain Mismatch', 'tripfery' );
		}
	...
	public function license_note_callback() {
		...
		$status  = sprintf(
			...
			__( 'Please keep in mind, you can activate one license in one domain, if you face any problem in activation, please contact our <a href="%s" target="_blank">Support Center</a>', 'tripfery' ),
			esc_url( $support )
		);
	...
	public function license_check_callback() {
		printf(
			...
			esc_html__( 'Activate License', 'tripfery' )
		);
	}
	...
	public function rtlc_verification() {
		...
		if ( $purchase_code ) {
			...
			$tripfery = $this->tripfery;
			$domain_name     = rtlc_get_domain_name();

			$api_url     = "{$rt_license_server}/?theme_name={$tripfery}&purchase_code=" . $purchase_code . '&domain_name=' . $domain_name;
