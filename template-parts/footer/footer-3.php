<?php
$tripfery_footer_column = TripferyTheme::$options['footer_column_3'];
...
if( !empty( TripferyTheme::$options['fbgimg3'] ) ) {
	$f1_bg = wp_get_attachment_image_src( TripferyTheme::$options['fbgimg3'], 'full', true );
	$footer_bg_img = $f1_bg[0];
}else {
	$footer_bg_img = TRIPFERY_ASSETS_URL . 'img/footer_bg.jpg';
}

if ( TripferyTheme::$options['footer_bgtype3'] == 'fbgcolor3' ) {
	$tripfery_bg = TripferyTheme::$options['fbgcolor3'];
} else {
	$tripfery_bg = 'url(' . $footer_bg_img . ') no-repeat center bottom / cover';
}
$bgc = $menu_on = $copyright_on = '';
if ( TripferyTheme::$options['footer_bgtype3'] == 'fbgimg3' ) {
...
<div class="footer-top-area <?php echo esc_attr( $bgc ); ?>" style="background:<?php echo esc_html( $tripfery_bg ); ?>">
	<?php if ( is_active_sidebar( 'footer-style-3-1' ) && TripferyTheme::$footer_area == 1 ) { ?>
