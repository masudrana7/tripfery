...
$tripfery_has_entry_meta  = ( TripferyTheme::$options['blog_date'] || TripferyTheme::$options['blog_author_name'] || TripferyTheme::$options['blog_comment_num'] || TripferyTheme::$options['blog_length'] && function_exists( 'tripfery_reading_time' ) || TripferyTheme::$options['blog_view'] && function_exists( 'tripfery_views' ) ) ? true : false;
							...
							<?php } else {
								if ( TripferyTheme::$options['display_no_preview_image'] == '1' ) {
									...
									elseif ( empty( TripferyTheme::$options['no_preview_image']['id'] ) ) {
										$thumbnail = '<img class="wp-post-image" src="'.TRIPFERY_ASSETS_URL.'img/noimage_960X520.jpg" alt="'. the_title_attribute( array( 'echo'=> false ) ) .'">';
									}
